...
var shortcuts = {
	...
	'Alt-2': toggleHeading2,
	'Alt-3': toggleHeading3,
	'Alt-4': toggleHeading4,
...
function getState(cm, pos) {
	...
	for(var i = 0; i < types.length; i++) {
		...
		if(data === "strong") {
			...
			var cursorIndex = -1, token;

			lineTokens.forEach(function (token, index) {
				if (token.start === stat.start && token.end === stat.end) {
					cursorIndex = index;
				}
			});

			while (token = lineTokens[cursorIndex--]) {
				if (token.type === 'comment' && token.string === '`' || token.string === '``') {
					ret.inlinecode = true;
					break;
				}
			}
...
// 光标是否在空白行
function isCursorAtEmptyLine(editor) {
	var cm = editor.codemirror;
	var pos = cm.getCursor('start');
	return cm.getLine(pos.line).length === 0;
}
...
function isCursorAtLineEnd(editor) {
	var cm = editor.codemirror;
	var pos = cm.getCursor('start');
	return pos.ch === cm.getLine(pos.line).length;
}

// 编辑器光标定位到末尾
function cursorToEnd(editor) {
	var cm = editor.codemirror;
	var pos = cm.getCursor('start');
	cm.doc.setCursor(pos.line, cm.getLine(pos.line).length);
}

// 换行
function sendCarriageReturn(editor, count) {
	var cm = editor.codemirror;
	cm.replaceSelection(Array((count || 1) + 1).join('\r\n'));
}
...
/**
 * Action for toggling inline code block.
 */
function toggleInlineCodeBlock(editor) {
	_toggleBlock(editor, "inlinecode", "`");
}
...
function toggleCodeBlock(editor) {
	if (isCursorAtEmptyLine(editor)) {
		sendCarriageReturn(editor, 1);
	} else {
		cursorToEnd(editor);
		sendCarriageReturn(editor, 2);
	}
...
/**
 * Action for drawing an exotic image.
 */
function drawExoticImage(editor, imageText) {
	var cm = editor.codemirror;
	var stat = getState(cm);
	_replaceSelection(cm, stat.image, imageText);
}
...
function toggleSideBySide(editor) {
	...
	cm.on("update", function() {
		var resultHTML = editor.options.previewRender(editor.value(), preview);
...
function _toggleBlock(editor, type, start_chars, end_chars) {
	...
	if(stat[type]) {
		...
		} else if(type == "italic" || type == "inlinecode") {
			startPoint.ch -= 1;
			if(startPoint !== endPoint) {
				endPoint.ch -= 1;
			}
...
SimpleMDE.prototype.render = function(el) {
	...
	keyMaps["Alt-J"] = function () {
		CodeMirror.commands.goLineDown(self.codemirror);
	};
	keyMaps["Alt-K"] = function () {
		CodeMirror.commands.goLineUp(self.codemirror);
	};
	keyMaps["Alt-L"] = function () {
		CodeMirror.commands.goCharRight(self.codemirror);
	};

	keyMaps["Shift-Alt-H"] = function () {
		self.codemirror.display.shift = true;
		CodeMirror.commands.goCharLeft(self.codemirror);
	};
	keyMaps["Shift-Alt-L"] = function () {
		self.codemirror.display.shift = true;
		CodeMirror.commands.goCharRight(self.codemirror);
	};
	keyMaps["Shift-Alt-J"] = function () {
		self.codemirror.display.shift = true;
		CodeMirror.commands.goLineDown(self.codemirror);
	};
	keyMaps["Shift-Alt-K"] = function () {
		self.codemirror.display.shift = true;
		CodeMirror.commands.goLineUp(self.codemirror);
	};

	keyMaps["Ctrl-Alt-H"] = function () {
		CodeMirror.commands.goWordLeft(self.codemirror);
	};
	keyMaps["Ctrl-Alt-L"] = function () {
		CodeMirror.commands.goWordRight(self.codemirror);
	};

	keyMaps["Shift-Ctrl-Alt-H"] = function () {
		self.codemirror.display.shift = true;
		CodeMirror.commands.goWordLeft(self.codemirror);
	};
	keyMaps["Shift-Ctrl-Alt-L"] = function () {
		self.codemirror.display.shift = true;
		CodeMirror.commands.goWordRight(self.codemirror);
	};

	keyMaps["Alt-A"] = function () {
		CodeMirror.commands.goLineStartSmart(self.codemirror);
	};
	keyMaps["Ctrl-E"] = function () {
		CodeMirror.commands.goLineEnd(self.codemirror);
	};
	keyMaps["Shift-Ctrl-Enter"] = function () {
		CodeMirror.commands.goLineStartSmart(self.codemirror);
		sendCarriageReturn(self);
		CodeMirror.commands.goLineUp(self.codemirror);
	};
	keyMaps["Ctrl-Enter"] = function () {
		CodeMirror.commands.goLineEnd(self.codemirror);
		CodeMirror.commands.newlineAndIndent(self.codemirror);
	};
	keyMaps["F9"] = function() {
...
SimpleMDE.toggleInlineCodeBlock = toggleInlineCodeBlock;
SimpleMDE.toggleCodeBlock = toggleCodeBlock;
SimpleMDE.toggleUnorderedList = toggleUnorderedList;
SimpleMDE.toggleOrderedList = toggleOrderedList;
SimpleMDE.drawLink = drawLink;
SimpleMDE.drawImage = drawImage;
SimpleMDE.drawExoticImage = drawExoticImage;
...
SimpleMDE.prototype.toggleInlineCodeBlock = function() {
	toggleInlineCodeBlock(this);
};
...
SimpleMDE.prototype.drawExoticImage = function() {
	drawExoticImage(this);
};
